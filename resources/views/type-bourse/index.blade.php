@php
    $pagetitle = 'Liste des Type Bourse(s)';
    $breadcrumbs = ['Liste des Type Bourse(s)' => route('type-bourses.index')];
@endphp
    ...
    <div class="">
        <div class="row">
            <div class="col-sm-12">
                <div class="card">
                    ...
                    <div class="card-body">
                      @if ($message = Session::get('success'))
                        ...
                        <div class="text-end">
                            <a href="{{ route('type-bourses.create') }}" class="btn btn-sm btn-primary rounded-05"> Nouveau</a>
                        </div>
                        <div class="col mb-2">
                            <h5 class="card-title text-dark fw-bolder mb-0">Type Bourse(s)</h5>
                            <span>Liste des Type Bourse(s)</span>
                            <hr>
                        </div>
                        <div class="table-responsive">
                            <table class="table table-striped table-hover datatable">
                                <thead class="thead">
									...
									<th >Codetypebourse</th>
									<th >Libelletypebourse</th>

                                        <th></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($typeBourses as $typeBourse)
                                        <tr>
                                            <td>{{ ++$i }}</td>
                                            
										<td >{{ $typeBourse->CodeTypeBourse }}</td>
										<td >{{ $typeBourse->LibelleTypeBourse }}</td>
                ...
                {!! $typeBourses->withQueryString()->links() !!}
            </div>
        </div>
    </div>
@endsection
